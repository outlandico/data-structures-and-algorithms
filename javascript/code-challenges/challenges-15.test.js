...
const screenForNames = (arr) => {
  return arr.filter(name => /^(Mr\.|Mrs\.|Ms\.|Dr\.)\s[A-Za-z\s]+$/.test(name));
};
...
let biggerThanLuke = (arr) => {
  ...
  return biggerCharacters.map(character => character.name).join(' - ');
};
...
const isSecure = (url) => {
  ...
  if (url.startsWith('https://')) {
    // Further validate if there is exactly one occurrence of 'https://' in the URL
    const regex = /^https:\/\/[^/]+\.[a-zA-Z]+(?:\/[^/]+)*$/;
...
const detectTicTacToeWin = (board) => {
  ...
  // Helper function to check if any row, column, or diagonal has a win
  const checkWin = (row1, col1, row2, col2, row3, col3) => {
    return allEqual([board[row1][col1], board[row2][col2], board[row3][col3]]);
  };

  // Check rows
  for (let i = 0; i < 3; i++) {
    if (checkWin(i, 0, i, 1, i, 2)) return true;
  }

  // Check columns
  for (let i = 0; i < 3; i++) {
    if (checkWin(0, i, 1, i, 2, i)) return true;
  }
