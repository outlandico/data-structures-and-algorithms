...
const sortStarWarsCharacters = (starWarsArr) => {
  // Custom comparison function to sort by height
  const compareByHeight = (a, b) => parseInt(b.height) - parseInt(a.height);

  // Sort the array by height
  starWarsArr.sort(compareByHeight);
...
const removeThree = (idx, arr) => {
  // Check if the index is valid and there are enough elements to remove
  if (idx >= 0 && idx + 2 < arr.length) {
    // Remove three items starting from the index
    arr.splice(idx, 3);
  }
  return arr;
};
...
console.log(removeThree(2, array)); // Removes 3, 4, and 5 from the array
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(ingredient => {
    // Find the index of the first space character in the ingredient string
    const spaceIndex = ingredient.indexOf(' ');
    // Find the index of the second space character in the ingredient string
    const secondSpaceIndex = ingredient.indexOf(' ', spaceIndex + 1);
    // Slice the ingredient string starting from the index after the second space
    const foodName = ingredient.slice(secondSpaceIndex + 1);
    // Push the extracted food name to the result array
...
// Example usage:
console.log(listFoods(gruffaloCrumble)); // Output will be an array of food names
